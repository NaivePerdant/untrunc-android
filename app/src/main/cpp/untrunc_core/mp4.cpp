...
Mp4::~Mp4() {
	delete root_atom_;
#ifdef ANDROID
	if (owns_current_file_) delete current_file_;
	// else caller owns it
#else
...
#ifdef ANDROID
void Mp4::parseOk(FileRead& file_read, bool accept_unhealthy) {
	filename_ok_ = file_read.filename_;
	auto& file = openFile(file_read);
	...
	if(root_atom_->atomByName("ctts"))
		logg(I, "Composition time offset atom found. Out of order samples possible.\n");

	if(root_atom_->atomByName("sdtp"))
		logg(I, "Sample dependency flag atom found. I and P frames might need to recover that info.\n");
...
#ifdef ANDROID
void Mp4::saveVideo(int fd) {
	if (tracks_.back().is_dummy_) tracks_.pop_back();

	if (g_log_mode >= I) {
		logg(I, "Found ", pkt_idx_, " packets ( ");
		for(const Track& t : tracks_){
			logg(I, t.codec_.name_, ": ", t.getNumSamples(), ' ');
		}
		logg(I, ")\n");
	}
	...
	for(Track& track : tracks_) {
		...
		int bmsec = track.getDurationInMs();
		logg(I, "Duration of ", cn, ": ", bmsec, " ms\n");
	}
	...
	for (Track& track : tracks_) {
		assert(track.chunks_.size(), track.codec_.name_, track.getNumSamples());
		for (auto& c : track.chunks_) c.off_ += offset;
		track.saveChunkOffsets();
	}

	// save to file via FD
	logg(I, "saving repaired file via fd\n");
	FileWrite file(fd);
...
#ifdef ANDROID
FileRead& Mp4::openFile(FileRead& file_read) {
	// Don't delete - caller owns the FileRead
	current_file_ = &file_read;
	owns_current_file_ = false;
	if (!current_file_->length())
		throw length_error("zero-length file");
	return *current_file_;
}
#endif
...
#ifdef ANDROID
void Mp4::repair(FileRead& file_read, int output_fd) {
	if (chkBadFFmpegVersion()) {
		return;
	}

	g_ignore_unknown = true;  // skip unrecognized data instead of aborting

	if (needDynStats()) {
		g_use_chunk_stats = true;
		genDynStats();
		checkForBadTracks();
		logg(I, "using dynamic stats\n");
	}
	else if (setDuplicateInfo()) {
		genTrackOrder();
	}
	...
	auto& fr = openFile(file_read);

	logg(V, "calling findMdat on truncated file..\n");
	auto mdat = findMdat(fr);
	logg(I, "reading mdat from truncated file ...\n");
	...
	while (chkOffset(offset)) {
		if (g_cancelled) {
			logg(I, "repair cancelled by user\n");
			break;
		}
		if (tryAll(offset)) continue;
		...
		if (g_ignore_unknown) {
			...
			if (g_muted) unmute();
			premature_end_ = true;
			premature_percentage_ = 100.0 * offset / mdat->contentSize();
			break;
		}
	}
	if (g_muted) unmute();
