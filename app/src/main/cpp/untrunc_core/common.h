...
extern volatile bool g_cancelled;
...
#ifdef ANDROID
	std::stringstream _android_ss;
	UNFOLD_PARAM_PACK(args, _android_ss);
	__android_log_print(ANDROID_LOG_INFO, "untrunc", "%s", _android_ss.str().c_str());
#else
...
void _logg(LogMode m, Args&&... x){
	...
	if (m == I) {
		std::stringstream _android_ss;
		_android_ss << "Info: ";
		UNFOLD_PARAM_PACK(x, _android_ss);
		__android_log_print(ANDROID_LOG_INFO, "untrunc", "%s", _android_ss.str().c_str());
	} else if (m == W || m == W2) {
		std::stringstream _android_ss;
		_android_ss << "Warning: ";
		UNFOLD_PARAM_PACK(x, _android_ss);
		__android_log_print(ANDROID_LOG_WARN, "untrunc", "%s", _android_ss.str().c_str());
	} else if (m <= E) {
		std::stringstream _android_ss;
		_android_ss << "Error: ";
		UNFOLD_PARAM_PACK(x, _android_ss);
		__android_log_print(ANDROID_LOG_ERROR, "untrunc", "%s", _android_ss.str().c_str());
		if (m == ET) {
			throw std::runtime_error(ss(std::forward<Args>(x)...));
		}
	} else {
...
std::string hexIf(Numeric x){
	if (g_off_as_hex) return ss(std::hex, "0x", x, std::dec);
	return ss(x);
}
...
#ifdef ANDROID
	std::string msg = std::string(file) + ":" + std::to_string(line) + ": " + fn + ": Assertion `" + expr_str + "' failed.";
	__android_log_print(ANDROID_LOG_ERROR, "untrunc", "%s", msg.c_str());
