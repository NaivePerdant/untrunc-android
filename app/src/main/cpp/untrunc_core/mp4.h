...
#ifdef ANDROID
	void parseOk(FileRead& file_read, bool accept_unhealthy=false); // FD-based overload
#endif
...
#ifdef ANDROID
	void repair(FileRead& file_read, int output_fd); // FD-based overload
#endif
...
#ifdef ANDROID
	FileRead& openFile(FileRead& file_read);
	bool owns_current_file_ = true;  // false when using FD-based openFile
#endif
