...
FileRead::FileRead(int fd, const string& display_name) {
	filename_ = display_name;
	fd_ = fd;  // save original fd for avformat_open_input
	// dup() to avoid double-close issues
	int duped_fd = dup(fd);
	if (duped_fd < 0) throw string("Could not dup file descriptor: ") + strerror(errno);

	file_ = fdopen(duped_fd, "rb");
	if (!file_) {
		close(duped_fd);
		throw string("Could not fdopen file descriptor for '") + display_name + "': " + strerror(errno);
	}
...
FileWrite::FileWrite(int fd) {
	int duped_fd = dup(fd);
	if (duped_fd < 0) throw string("Could not dup file descriptor: ") + strerror(errno);

	file_ = fdopen(duped_fd, "wb");
	if (!file_) {
		close(duped_fd);
		throw string("Could not fdopen file descriptor for writing: ") + strerror(errno);
	}
}
