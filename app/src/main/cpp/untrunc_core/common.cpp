...
void resetGlobalState() {
	g_log_mode = LogMode::I;
	g_max_partsize_default = 1<<23;  // 8MiB
	g_max_partsize = 0;
	g_max_buf_sz_needed = 1<<19;  // 512kiB
	g_interactive = true;
	g_muted = false;
	g_ignore_unknown = false;
	g_stretch_video = false;
	g_show_tracks = false;
	g_dont_write = false;
	g_use_chunk_stats = false;
	g_dont_exclude = false;
	g_dump_repaired = false;
	g_search_mdat = false;
	g_strict_nal_frame_check = true;
	g_allow_large_sample = false;
	g_ignore_forbidden_nal_bit = false;
	g_dont_omit = false;
	g_noise_buffer_active = false;
	g_ignore_out_of_bound_chunks = false;
	g_skip_existing = false;
	g_ignore_keyframe_mismatch = false;
	g_skip_nal_filler_data = false;
	g_rsv_ben_mode = false;
	g_off_as_hex = true;
	g_fast_assert = false;
	g_no_ctts = false;
	g_is_gui = false;
...
void hitEnterToContinue(bool new_line) {
	if (g_interactive) {
		logg(V, "  [[Hit enter to continue]]", (new_line? "\n" : ""));
#ifndef ANDROID
		getchar();
#endif
	}
...
void chkHiddenWarnings() {
	if (g_num_w2 && g_log_mode >= W) {
		logg(V, string(10, ' '), '\n');
		logg(V, g_num_w2, " warnings were hidden!\n");
	}
	g_num_w2 = 0;
}
...
//	if (first_failed == data.size() - 1)  // last value might be shorter
	...
	if (g_log_mode >= V) {
		logg(V, "first_failed: ", first_failed, " of ", orig_sz, '\n');
		std::string order_str = "order: ";
		for (auto& p : data) order_str += ss("(", p.first, ", ", p.second, ") ");
		logg(V, order_str, '\n');
	}
...
vector<int> findOrderSimple(const vector<pair<int, int>>& data) {
	...
	if (g_log_mode >= V) {
		logg(V, "first_failed: ", first_failed, " of ", data.size(), '\n');
		std::string order_str = "simpleOrder: ";
		for (auto& x : result) order_str += ss(x, " ");
		logg(V, order_str, '\n');
	}
...
void showStacktrace() {
#ifdef ANDROID
    __android_log_print(ANDROID_LOG_ERROR, "untrunc", "Assertion failed");
#elif defined(_WIN32)
